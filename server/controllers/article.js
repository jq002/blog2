...
class ArticleController {
  static async createArticle(ctx) {
    ...
    if(!abstract){
      ctx.throw(400,'abstract is not exist')
    }
    if (!content) {
  ...
  static async getAllPublishArticles(ctx) {
    ...
    const searchTitle=ctx.query.searchTitle;
    let skip = 0;
    let articleArr;
    let allPage;
    let allNum;
    ...
    if(searchTitle){
      queryStr['title']=new RegExp(searchTitle);//模糊查询参数
    }
    query=Article.find(queryStr);
    if (limit && page !== 0) {
      skip = limit * (page - 1)

      query.limit(limit).skip(skip);
    }
    articleArr = await query.populate("tags").sort({
      createTime: -1
    }).exec().catch(err => {
      ctx.throw(500, 'server error !!!')
    });
